...
# ── Load your original script without modifying it ───────────
try:
...
# ── Now read the same data and save to DB ────────────────────
...
total_buy_val  = buy_val_tqqq + buy_val_agg
total_curr_val = curr_val_tqqq + curr_val_agg
...
history    = (s_tqqq * hist_data["TQQQ"]) + (s_agg * hist_data["AGG"])
...
# ── Prepare and Send Telegram Report ─────────────────────────
tg_report = f"""
📊 *Portfolio Analysis Report*
_{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}_

💰 *Total Value:* `${total_curr_val:,.2f}`
📈 *Total Return:* `{total_return*100:+.2f}%` (${total_profit:,.2f})

🔹 *TQQQ:* ${curr_tqqq:,.2f} ({return_tqqq*100:+.2f}%)
🔹 *AGG:* ${curr_agg:,.2f} ({return_agg*100:+.2f}%)

🎯 *Quarterly Target (9%):*
• Status: `{q_perf*100:+.2f}%`
